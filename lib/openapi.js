...
import { getCustomMethods } from './custom-methods.js';
...
function ignoreService(service, path, includeConfig, ignoreConfig) {
  ...
  if (includeConfig.paths.length) {
    includedByPath = includeConfig.paths.some(
      (pathToInclude) => path.match(pathToInclude) !== null
    );
  }
...
class OpenApiGenerator {
  ...
  getSchemaNames(serviceSchemaNames = {}, defaultSchemaNames = {}) {
    return {
      ...
      pagination:
        serviceSchemaNames.pagination ||
        defaultSchemaNames.pagination ||
  ...
  addService(service, path, serviceOptions) {
    ...
    const operationArgs = this.getOperationArgs({
      service,
      path,
      config: this.config,
      apiPath,
    ...
    if (typeof this.config.defaults.getOperationArgs === 'function') {
      Object.assign(
        operationArgs,
        this.config.defaults.getOperationArgs({
          service,
          path,
          config: this.config,
          apiPath,
    ...
    const multiOperations =
      doc.multi || this.config.defaults.multi || determineMultiOperations(service);
    ...
    const addMethodToSpecs = (
      ...
      if (customMethod) {
        ...
        swaggerPath = swaggerPath.replace(
          ':__feathersId',
          utils.idPathParameters(idName, idSeparator)
        );
        const generator =
          typeof this.config.defaults.operationGenerators.custom === 'function'
        ...
        const generator =
          typeof this.config.defaults.operationGenerators[method] === 'function'
            ? this.config.defaults.operationGenerators[method]
            : this.operationDefaults[method];
        defaults = generator(
          methodIdName ? { ...defaultArgumentObject, idName: methodIdName } : defaultArgumentObject
        );
      }
      ...
      pathObj[swaggerPath][httpMethod] = utils.operation(
        method,
        service,
        defaults,
        this.operationSpecDefaults
      );
    ...
    getCustomMethods(this.app, service, defaultMethods, basePath, doc, serviceOptions).forEach(
