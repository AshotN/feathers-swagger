...
import { checkImport, isExpressApp, isKoaApp, requireOrFail } from './helpers.js';
...
const createCustomMethodsFetcher = async () => {
  if (await checkImport('@feathersjs/express/rest', 'HTTP_METHOD')) {
    ...
    const { HTTP_METHOD, httpMethod } = await import('@feathersjs/express/rest').then(
      (mod) => mod.default
    );
  ...
  if (await checkImport('@feathersjs/feathers', 'getServiceOptions')) {
    const { getServiceOptions, defaultServiceMethods } = await import('@feathersjs/feathers').then(
      (mod) => mod.default
    );
    const http = await import('@feathersjs/transport-commons').then((mod) => mod.http);
...
const { getCustomMethods, registerCustomMethod, customMethodsHandler } =
  await createCustomMethodsFetcher();
